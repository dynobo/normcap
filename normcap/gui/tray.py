...
class SystemTray(QtWidgets.QSystemTrayIcon):
    ...
    def __init__(self, parent: QtCore.QObject, args: dict[str, Any]) -> None:
        ...
        self.delayed_exit_timer.timeout.connect(self._exit_application)
    ...
    def _color_tray_icon(self) -> None:
        if sizes := self.icon().availableSizes():
            self.icon_pixmap = self.icon().pixmap(sizes[-1])
            self.icon_mask = self.icon_pixmap.createMaskFromColor(
                QtGui.QColor("transparent"), QtCore.Qt.MaskMode.MaskInColor
            )
            self.icon_pixmap.fill(QtGui.QColor(str(self.settings.value("color"))))
            self.icon_pixmap.setMask(self.icon_mask)
            self.setIcon(QtGui.QIcon(self.icon_pixmap))
    ...
    def _crop_image(self, grab_info: tuple[Rect, int]) -> None:
        ...
        utils._save_image_in_temp_folder(self.capture.image, postfix="_cropped")
    ...
    def _capture_to_ocr(self) -> None:
        ...
        utils._save_image_in_temp_folder(ocr_result.image, postfix="_enhanced")
    ...
    @QtCore.Slot(bool)
    def _close_windows(self, delayed_exit: bool = False) -> None:
    ...
    def _set_signals(self) -> None:
        ...
        self.com.on_copied_to_clipboard.connect(self._close_windows)
    ...
    def _take_screenshots(self, delay: bool) -> list[QtGui.QImage]:
        ...
        for idx, screenshot in enumerate(screens):
            utils._save_image_in_temp_folder(screenshot, postfix=f"_raw_screen{idx}")
    ...
    def _populate_context_menu_entries(self) -> None:
        ...
        action.triggered.connect(self._exit_application)
        self.tray_menu.addAction(action)
    ...
    def _create_menu_button(self) -> QtWidgets.QLayout:
        ...
        settings_menu.com.on_close.connect(self._close_windows)
