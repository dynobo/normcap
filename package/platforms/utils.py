...
def build_wl_clipboard(self, app):  # pylint: disable=unused-argument
    print("Building wl-clipboard...", flush=True)
    self.tools[app].app_context.run(
        "git clone https://github.com/bugaevc/wl-clipboard.git".split(),
        check=True,
    )
    self.tools[app].app_context.run("meson build wl-clipboard".split(), check=True)
    self.tools[app].app_context.run(
        "ninja -C build".split(), capture_output=True, text=True
    )
